...
pub struct DsData {
    ...
    rooms: Arc<RwLock<HashMap<Vec<u8>, Sender<String>>>>,
}
...
async fn send_welcome(mut body: Payload, data: web::Data<DsData>) -> impl Responder {
    ...
    println!("Storing welcome message: {:?}", welcome.secrets());
...
async fn msg_send(mut body: Payload, data: web::Data<DsData>) -> impl Responder {
    ...
    println!("group message: {:?}", group_msg);
    ...
    if protocol_msg.is_handshake_message() {
        ...
        println!("group_id: {:?}, epoch: {}", group_id, epoch);
...
#[get("/long_poll")]
async fn long_polling(req: HttpRequest) -> impl Responder {
    sleep(Duration::from_secs(5)).await;
    println!("Long polling response");

    actix_web::HttpResponse::Ok().body("Long polling response")
}
...
pub async fn ws_router(ws_data: web::Data<DsData>) -> std::io::Result<()> {
    let listener = tokio::net::TcpListener::bind("0.0.0.0:6543").await.unwrap();
    println!("Listening on 6543 port");

    while let Ok((stream, addr)) = listener.accept().await {
        println!("{} connected", addr);

        let rooms = ws_data.rooms.clone();

        tokio::spawn(async move {
            handle_stream(stream, rooms.clone()).await;
        });
    }
    Ok(())
}

async fn handle_stream(stream: TcpStream, rooms: Arc<RwLock<HashMap<Vec<u8>, Sender<String>>>>) {
    let current_room: Vec<u8> = Vec::from("chat");
    ...
    if let Ok(mut ws_stream) = accept_async(stream).await {
        let msg = "Welcome to the chat!".to_string();
        ws_stream
        ...
        loop {
            ...
            tokio::select! {
                ...
                recv_msg = receiver.recv() => {
                    match recv_msg {
                        Ok(msg) => {
                            ws_stream.send(tungstenite::Message::Text(msg)).await.unwrap();
                        }
                        Err(e) => {
                            println!("Error: {}", e);
                            break;
